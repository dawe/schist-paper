...
Where $Q_x$ is modularity at level $x$. We tabulated values arising from both the approaches in Table \ref{table_optimal}, together with the $i^D$ that is defined similarly to Eq. \ref{Equation_RMT} using the number of known annotated groups $D$ in place of $\hat{k}$

\begin{table}[h!]
\centering
 \begin{tabular}{|| l c c c c c c c c c ||}
 \hline
 \textbf{Dataset} & \textbf{Cells} & \textbf{D} & \textbf{D} & \textbf{nD} & \textbf{$\hat{k}$} & \textbf{$i$} & \textbf{$B_i$} & \textbf{Annotated groups} & \textbf{Annotated groups} \\ [0.5ex] 

 \hline\hline
% sc-mixology \cite{Tian_2019} & 860 & 5 & 00:08 & 00:55 & 01:23 \\ 
